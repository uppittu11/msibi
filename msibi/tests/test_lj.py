...
def test_full_lj():
    ...
    return pairs[0].potential

def test_full_lj_big_chunk():
    """Test the full Lennard-Jones optimization with, loading the trajectories iteratively. """
    # Root directory for the Lennard-Jones optimization.
    lj = os.path.abspath(os.path.join(os.path.dirname(__file__), 'lj-longer'))

    # Set up global parameters.
    rdf_cutoff = 5.0
    opt = MSIBI(rdf_cutoff=rdf_cutoff, n_rdf_points=101, pot_cutoff=3.0,
                smooth_rdfs=True, base_dir=lj, max_distance_pairs=2e6)
...
if __name__ == "__main__":
    small_chunks = test_full_lj()
    large_chunks = test_full_lj_big_chunk()
    assert(np.allclose(small_chunks, large_chunks, atol=0.1))
